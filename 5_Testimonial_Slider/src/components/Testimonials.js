...
const Testimonials = (props) => {
    ...
    function leftShiftHandler() {
        if(index - 1 < 0) { // when it get -1
            setIndex(reviews.length - 1);// it set to last one (n-1)
        }
        else {
            setIndex(index - 1);//shift to left
        }
    }

    function rightShiftHandler() {
        if(index + 1 >=reviews.length) { //when it more than size
            setIndex(0);//set to 1st
        }
        else {
            setIndex(index+1);//shit ro right
        }
